...
const NAV_ITEMS = [
  ...
  { href: 'https://www.tsukisurlamerwedding.com/book-online', en: 'Fair', jp: 'フェア', ext: true }];
  ...
  return (
    ...
    <nav className={`nav${solid ? ' solid' : ''}${menuOpen ? ' menu-open' : ''}`}>
      ...
      <div className="nav-links">
        {NAV_ITEMS.map((n) => <a key={n.href} href={n.href} target={n.ext ? '_blank' : undefined} rel={n.ext ? 'noopener' : undefined}>{n.en}</a>)}
      </div>
      <div className="nav-cta">
        <a className="nav-ig" href="https://www.instagram.com/tsuki_wedding_tokyo?igsh=MXJoYTE3NDlscTFhcg==" target="_blank" rel="noopener" aria-label="Instagram">
    ...
    <div className={`mobile-menu${menuOpen ? ' open' : ''}`} role="dialog" aria-modal="true">
      <div className="mm-inner">
        <nav className="mm-links">
          {NAV_ITEMS.map((n, i) =>
            <a key={n.href} href={n.href} target={n.ext ? '_blank' : undefined} rel={n.ext ? 'noopener' : undefined} style={{ transitionDelay: (0.05 + i * 0.04) + 's' }}
               onClick={() => setMenuOpen(false)}>
              <span className="mm-en">{n.en}</span>
            </a>)}
        </nav>
        <div className="mm-foot">
          <a href="https://www.tsukisurlamerwedding.com/book-online" target="_blank" rel="noopener" className="mm-pill" onClick={() => setMenuOpen(false)}>Reservation</a>
        </div>
      </div>
    </div>
    </React.Fragment>);
  ...
  return (
    <section id="concept" className="concept-page" ref={ref}>
      ...
      <div className="cp-statement">
        <h2 className="cp-title fade">おもてなしは<br />「東京」という<br />街のエレガンス</h2>
      </div>

      <div className="cp-lead-block fade">
        <h3 className="cp-lead-head">すべてが語り合う理由になる場所</h3>
        <div className="cp-lead-body">
          <p>ツキシュールラメールを満たしている空気は、<br />「東京」という街のエレガンス。<br />イルミネーションきらめく夜景、青空に映えるレインボーブリッジ、海上を行き交う客船…<br />そんな東京の絶景を眺めていると、<br />流れる音楽も、運ばれる一皿も、<br />恋人のおめかしも、五感で感じるすべてが語り合いたくなる理由になってしまう。<br />誰もが自然と洗練された気分になってしまう。</p>
          <p>私たちがご用意しているのは、<br />そんなひとときです。</p>
          <p>大切な皆さまへ、<br />「東京」という街のエレガンスを贈ること。<br />それが、ツキシュールラメールで<br />叶う最高のおもてなしです。</p>
        </div>
      </div>
      ...
      <div className="cp-links">
        <div className="cp-link fade">
          ...
          <a className="cp-link-btn" href="https://www.tsukisurlamerwedding.com/book-online" target="_blank" rel="noopener">Reservation</a>
        </div>
...
function Banquet() {
  ...
  return (
    <section id="banquet" className="concept-page" ref={ref}>
      ...
      <div className="cp-lead-block fade">
        <h3 className="cp-lead-head">リゾート気分で眺める<br />大都会のパノラマ</h3>
        <div className="cp-lead-body">
          <p>都心の一等地にいながら、喧騒を忘れ、<br />穏やかなプライベートタイムを<br />お過ごしいただけます。<br />眺めているのは躍動する大都会のパノラマ。<br />それなのに、ここに流れているのは、<br />リゾート地にいるような<br />リラックスした時間。</p>
          <p>そんな鮮やかなコントラストこそ、<br />訪れる人すべてを<br />ロマンチストにしてしまうエレガンスなのです。</p>
        </div>
      </div>
...
{ id: 'chapel', idx: '1', en: 'Chapel', jp: '凛として', img: 'assets/ceremony-chapel.jpg',
  ...
  desc: '家族や親友だけに見守られ、<br />一言ひとつの言葉の重みが<br />まっすぐ伝わる距離感。<br />心の深いところで幸せを実感できる時間。',
...
{ id: 'banquet', idx: '2', en: 'Banquet', jp: '自然体', img: 'assets/ceremony-banquet.jpg',
  ...
  desc: 'パーティ会場での挙式は、<br />リラックスした状態で<br />ゲストと一体になれるスタイル。<br />１日中、ずっと和やかなテンションで過ごしたい人へ。',
  capLabel: 'capacity', capValue: '110 Guests',
  tag: 'Soft & Relax' },
{ id: 'shrine', idx: '3', en: 'Shrine', jp: '厳かに', img: 'assets/ceremony-shrine.jpg',
  head: '静と動のコントラストを楽しむ',
  desc: '厳かで静謐に包まれた神社挙式は<br />開放感のあるパーティとのコントラストが抜群。<br />メリハリのある１日に。',
  ...
  return (
    <section id="ceremony" className="concept-page" ref={ref}>
      ...
      <div className="cp-lead-block fade">
        ...
        <div className="cp-lead-body">
          <p>挙式は３つのスタイルをご用意。<br />ふたりの感性、ゲストの顔ぶれに合わせて、<br />しっくりくるスタイルをお選びいただけます。<br />それぞれ、教会式・人前式が選べます。</p>
        </div>
      </div>
...
function Timeline() {
  ...
  return (
    <section id="day" className="timeline" ref={ref}>
      <div className="tl-head fade">
        <p className="tl-sub">瞬きの間すら<br />愛おしい１日を</p>
      ...
      <div className="tl">
        {TIMELINE.map((row, i) => {
          ...
          const Text =
          <>
              <div className="tl-step">STEP {String(i + 1).padStart(2, '0')}</div>
  ...
  return (
    <section id="fair" className="fair" ref={ref}>
      ...
      <div className="fair-cal-wrap fade">
        ...
        <div className="fair-day">
          ...
          <div className="fair-card" key={f.slug || fi}>
            ...
            <a className="fair-cta" href="https://www.tsukisurlamerwedding.com/book-online" target="_blank" rel="noopener">
      ...
      <div className="fair-foot fade">
        ...
        <a className="btn-primary" href="https://www.tsukisurlamerwedding.com/book-online" target="_blank" rel="noopener">
    ...
    <footer>
      <div className="ft-inner">
        ...
        <nav className="ft-links">
          <a href="https://www.tsukisurlamerwedding.com/book-online" target="_blank" rel="noopener">ご予約</a>
          <a href="https://www.tsukisurlamerwedding.com/book-online" target="_blank" rel="noopener">お問い合わせ</a>
          <a href="https://hyperdrive.co.jp/privacy/" target="_blank" rel="noopener">プライバシーポリシー</a>
          <a href="https://hyperdrive.co.jp/" target="_blank" rel="noopener">会社概要</a>
        </nav>
...
function Gallery() {
  ...
  return (
    <section id="gallery" className="concept-page" ref={ref}>
      ...
      <div className="cp-lead-block fade">
        <div className="cp-lead-body">
          <p>チャペル、パーティ、お料理、テラス…、<br />ツキシュールラメールで生まれる情景を、そのままご覧ください。</p>
        </div>
      </div>
      <div className="gallery-grid">
        {GALLERY_IMAGES.map((src, i) =>
        <button key={i} type="button" className="gallery-cell fade" style={{ backgroundImage: `url('${src}')` }} onClick={() => setZoom(src)} aria-label="拡大表示" />
        )}
      </div>
      {zoom &&
      <div className="gal-lightbox" onClick={() => setZoom(null)}>
        <img src={zoom} alt="" />
        <button type="button" className="gal-close" aria-label="閉じる">×</button>
      </div>}
  ...
  return (
    <section id="faq" className="concept-page" ref={ref}>
      ...
      <div className="faq-tabs">
        {FAQ_GROUPS.map((g, gi) =>
        <a key={gi} className="faq-tab" href={`#faq-cat-${gi}`} onClick={(e) => { e.preventDefault(); const el = document.getElementById('faq-cat-' + gi); if (el) window.scrollTo({ top: el.getBoundingClientRect().top + window.pageYOffset - 90, behavior: 'smooth' }); }}>{g.cat}</a>
        )}
      </div>
      <div className="faq-wrap fade">
        {FAQ_GROUPS.map((g, gi) =>
        <div key={gi} className="faq-group" id={`faq-cat-${gi}`}>
...
const PAGES = [
  ...
  { slug: 'dresses',  en: 'Dress',    jp: 'ドレス',        Comp: Dresses,  img: 'assets/dress-main.avif', noHeroImg: true,
    ...
    lead: '約120店舗・5,000着のラインナップから、<br />あなただけの特別な一着を。' },
  { slug: 'day',      en: 'A Day',    jp: '当日の流れ',     Comp: Timeline, img: 'assets/day-kv.jpg', noHeroImg: true,
    lead: '朝の支度からお見送りまで。<br />瞬きするのが惜しい、一日の物語。' },
  { slug: 'gallery',  en: 'Gallery',  jp: 'ギャラリー',      Comp: Gallery,  img: 'assets/banquet.jpg', noHeader: true,
    lead: 'チャペル、パーティ、お料理、テラス…<br />ここで生まれる情景をひとコマずつ。' },
  { slug: 'faq',      en: 'F＆Q',      jp: 'よくある質問',   Comp: FAQ,      img: 'assets/concept-bg.jpg', noHeader: true,
    lead: 'ご見学・お料理・ドレスなど、<br />初めての方からよくいただくご質問にお答えします。' },
  { slug: 'fair',     en: 'Fair',     jp: 'ブライダルフェア', Comp: Fair,    img: 'assets/fair-kv.jpg', ext: 'https://www.tsukisurlamerwedding.com/book-online',
    lead: '試食や会場見学ができるブライダルフェアを開催。<br />まずはお気軽にご体験ください。' },
  ...
  return (
    <div className="home-seq" ref={ref}>
      ...
      <section key={p.slug} className={"seq-item" + (i >= 5 ? " sm" : "")} data-idx={i}>
        <a className="seq-title" href={p.ext || `#/${p.slug}`} target={p.ext ? '_blank' : undefined} rel={p.ext ? 'noopener' : undefined}>{p.en}</a>
        {p.slides ?
        <a className="seq-img seq-slides" href={p.ext || `#/${p.slug}`} target={p.ext ? '_blank' : undefined} rel={p.ext ? 'noopener' : undefined} aria-label={p.en}>
          {p.slides.map((s, k) =>
          <span key={k} className={"seq-slide" + (k === 0 ? " on" : "")} style={{ backgroundImage: `url('${s}')` }} />
          )}
        </a> :
        <a className={"seq-img" + (p.pano ? " pano" : "")} href={p.ext || `#/${p.slug}`} target={p.ext ? '_blank' : undefined} rel={p.ext ? 'noopener' : undefined} style={{ backgroundImage: `url('${p.img}')` }} aria-label={p.en} />}
        ...
        <div className="seq-foot">
          ...
          <a className="seq-more" href={p.ext || `#/${p.slug}`} target={p.ext ? '_blank' : undefined} rel={p.ext ? 'noopener' : undefined}>View More</a>
        </div>
