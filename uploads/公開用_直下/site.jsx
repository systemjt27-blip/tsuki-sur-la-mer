...
function HomeIndex() {
  ...
  useEffect(() => {
    const el = ref.current && ref.current.querySelector('.seq-img.pano');
    if (!el) return;
    let dragging = false, sx = 0, pos = 50, startPos = 50, moved = false;
    el.style.backgroundPosition = pos + '% center';
    el.style.backgroundSize = 'auto 100%';
    const down = (e) => { dragging = true; moved = false; sx = (e.touches ? e.touches[0].clientX : e.clientX); startPos = pos; el.classList.add('grabbing'); };
    const move = (e) => {
      if (!dragging) return;
    ...
    const clk = (e) => { if (moved) { e.preventDefault(); moved = false; } };
    el.addEventListener('pointerdown', down);
    el.addEventListener('click', clk);
    window.addEventListener('pointermove', move, { passive: false });
    window.addEventListener('pointerup', up);
    return () => { el.removeEventListener('pointerdown', down); el.removeEventListener('click', clk); window.removeEventListener('pointermove', move); window.removeEventListener('pointerup', up); };
  }, []);
  return (
