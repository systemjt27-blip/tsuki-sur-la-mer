...
function HomeIndex() {
  ...
  useEffect(() => {
    ...
    const cleanups = groups.map((g) => {
      ...
      let idx = 0;
      const show = (n) => { slides[idx].classList.remove('on'); idx = (n + slides.length) % slides.length; slides[idx].classList.add('on'); };
      const navs = Array.from(g.querySelectorAll('.seq-nav'));
      const onNav = (e) => { e.preventDefault(); e.stopPropagation(); show(idx + Number(e.currentTarget.getAttribute('data-dir'))); };
      navs.forEach((b) => { b.addEventListener('click', onNav); b.addEventListener('touchend', onNav); });
      return () => navs.forEach((b) => { b.removeEventListener('click', onNav); b.removeEventListener('touchend', onNav); });
    });
    return () => cleanups.forEach((fn) => fn());
  }, []);
  return (
    <div className="home-seq" ref={ref}>
      ...
      <section key={p.slug} className={"seq-item" + (i >= 5 ? " sm" : "")} data-idx={i}>
        ...
        <a className="seq-img seq-slides" href={`#/${p.slug}`} aria-label={p.en}>
          ...
          <button type="button" className="seq-nav prev" data-dir="-1" aria-label="前へ">‹</button>
          <button type="button" className="seq-nav next" data-dir="1" aria-label="次へ">›</button>
        </a> :
